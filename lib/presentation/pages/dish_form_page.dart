...
class _DishFormPageState extends State<DishFormPage> {
  ...
  @override
  void dispose() { _nameCtrl.dispose(); _priceCtrl.dispose(); _descCtrl.dispose(); _imageUrl.dispose(); super.dispose(); }
  ...
  Future<void> _pickImage() async {
    try {
      ...
      if (perm != PermissionState.authorized && perm != PermissionState.limited) {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(_t('permission_needed'))));
        return;
      }
      final files = await AssetPicker.pickAssets(context, pickerConfig: const AssetPickerConfig(maxAssets: 1, requestType: RequestType.image));
      if (files == null || files.isEmpty) return;
      final file = await files.first.file;
      if (file == null) return;
      final cropped = await ImageCropper().cropImage(
        sourcePath: file.path, aspectRatio: const CropAspectRatio(ratioX: 4, ratioY: 3),
        uiSettings: [AndroidUiSettings(toolbarTitle: _t('crop_image'), toolbarColor: AppColors.primary), IOSUiSettings(title: _t('crop_image'))],
      );
  ...
  void _save() {
    ...
    final r = Recipe(
      ...
      imageUrl: _imageUrl.value.isEmpty ? 'https://picsum.photos/seed/${_nameCtrl.text.trim()}/400/300' : _imageUrl.value,
    );
  ...
  Widget build(BuildContext context) {
    return Directionality(textDirection: TextDirection.rtl, child: Scaffold(
      ...
      body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
        DishFormFields(
          formKey: _formKey, nameCtrl: _nameCtrl, priceCtrl: _priceCtrl, descCtrl: _descCtrl,
          imageUrl: _imageUrl, initialCategory: _category, t: _t, isEditing: _isEditing, onPickImage: _pickImage, onCategoryChanged: (v) => _category = v,
        ),
        const SizedBox(height: 24),
        SizedBox(width: double.infinity, height: 48,
          child: ElevatedButton(onPressed: _save,
            child: Text(_isEditing ? _t('update_btn') : _t('add_btn'), style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)))),
      ]))),
    ));
  }
}
