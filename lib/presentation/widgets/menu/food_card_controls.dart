      ...
      if (quantity > 0)
        Column(mainAxisSize: MainAxisSize.min, children: [
          FittedBox(child: Row(mainAxisSize: MainAxisSize.min, children: [
            InkWell(onTap: () { HapticFeedback.lightImpact(); onDecrement?.call(); }, borderRadius: BorderRadius.circular(isTablet ? 14 : 12),
              child: Container(width: btnW, height: isTablet ? 28 : 25, alignment: Alignment.center,
                decoration: BoxDecoration(color: AppColors.primary,
                  borderRadius: BorderRadius.circular(isTablet ? 14 : 12)),
                child: Icon(Icons.remove, size: isTablet ? 22 : 20, color: cs.onPrimary))),
            SizedBox(width: gap),
            Text('$quantity', style: TextStyle(fontWeight: FontWeight.w800, fontSize: isTablet ? 17 : 15, color: AppColors.primary)),
            SizedBox(width: gap),
            InkWell(onTap: () { HapticFeedback.lightImpact(); onIncrement?.call(); }, borderRadius: BorderRadius.circular(isTablet ? 14 : 12),
              child: Container(width: btnW, height: isTablet ? 28 : 25, alignment: Alignment.center,
                decoration: BoxDecoration(color: AppColors.primary,
                  borderRadius: BorderRadius.circular(isTablet ? 14 : 12)),
                child: Icon(Icons.add, size: isTablet ? 22 : 20, color: cs.onPrimary))),
          ])),
