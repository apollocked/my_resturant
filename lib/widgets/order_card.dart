...
class OrderCard extends StatelessWidget {
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          if (showTime)
            ...
            TextButton.icon(
              ...
              label: Text(Tr.get('again', locale), style: const TextStyle(color: AppTheme.textSecondary, fontSize: 12, fontWeight: FontWeight.w600)),
            ),
          Text('${order.totalPrice.toInt()} ${Tr.get('currency_suffix', locale)}', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 15, color: AppTheme.primary)),
        ]),
      ])));
  }
  ...
  Widget _timeline(OrderStatus current, ColorScheme cs, Locale locale) {
    return Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(Tr.get('timeline_served', locale), style: TextStyle(fontSize: 9, color: current == OrderStatus.served ? AppTheme.success : AppTheme.textSecondary)),
        Text(Tr.get('timeline_preparing', locale), style: TextStyle(fontSize: 9, color: current == OrderStatus.preparing || current == OrderStatus.served ? _colors[OrderStatus.preparing] : AppTheme.textSecondary)),
        Text(Tr.get('timeline_pending', locale), style: TextStyle(fontSize: 9, color: current == OrderStatus.pending ? _colors[OrderStatus.pending] : AppTheme.textSecondary)),
      ]),
    ]);
  }
