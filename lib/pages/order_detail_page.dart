...
class OrderDetailPage extends StatelessWidget {
  ...
  static const _colors = {OrderStatus.pending: Colors.orange, OrderStatus.preparing: Colors.blue, OrderStatus.served: AppTheme.success};
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text('${order.totalPrice.toInt()} ${t('currency_suffix')}', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 22, color: AppTheme.primary)),
          ]),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text(t('foods'), style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: AppTheme.textPrimary)),
          ]),
    ...
    return Card(margin: const EdgeInsets.only(bottom: 8),
      child: Padding(padding: const EdgeInsets.all(12), child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Text('${item.recipe.price.toInt()} ${t('currency_suffix')}', style: const TextStyle(fontSize: 11, color: AppTheme.textSecondary)),
        ])),
        Text('${item.totalPrice.toInt()} ${t('currency_suffix')}', style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: AppTheme.textPrimary)),
      ])));
  }
  ...
  Widget _buildTimeline(OrderStatus current, String Function(String) t, ColorScheme cs) {
    return Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(t('timeline_served'), style: TextStyle(fontSize: 9, color: current == OrderStatus.served ? AppTheme.success : AppTheme.textSecondary)),
        Text(t('timeline_preparing'), style: TextStyle(fontSize: 9, color: current == OrderStatus.preparing || current == OrderStatus.served ? _colors[OrderStatus.preparing] : AppTheme.textSecondary)),
        Text(t('timeline_pending'), style: TextStyle(fontSize: 9, color: current == OrderStatus.pending ? _colors[OrderStatus.pending] : AppTheme.textSecondary)),
      ]),
    ]);
  }
